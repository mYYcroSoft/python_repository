"""
Úloha z https://exercism.org/tracks/python/exercises/hangman
...
def emoji_tryes():
    tryes_emoji = ""
    player_tryes_count = max_try - player_tryes
    for x in range(player_tryes_count): 
        tryes_emoji = tryes_emoji + ' ❤️ '     
    return tryes_emoji


def array_text_to_string():
    text_string = ""
    for x in list(word_string):
        text_string = text_string + x
    return text_string
def print_stats():
    
    print(len(passed_letter))
    print(f"Použitá písmena: {used_letters}")
    print(f"Pokusy: {emoji_tryes()}")
    print(f'Písmena: {array_text_to_string()}')
